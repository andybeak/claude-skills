...
allowed-tools: Bash(npx agent-reviews *) Bash(pnpm dlx agent-reviews *) Bash(yarn dlx agent-reviews *) Bash(bunx agent-reviews *) Bash(gh pr view *) Bash(git config *) Bash(git add *) Bash(git commit *) Bash(git push *)
metadata:
  author: pbakaus
  version: "1.0.1"
...
### Step 1: Fetch All Inline Bot Comments (Expanded)
...
**IMPORTANT:** `agent-reviews` only fetches **inline code review comments**. It does NOT fetch bot findings posted in the PR-level review body. Many bots (especially CodeRabbit) post the bulk of their findings in the review body when the number of issues is too large to post inline. Always proceed to Step 2 regardless of whether Step 1 returned comments.

### Step 2: Fetch PR Review Bodies from Bots

Run `gh pr view --json reviews --jq '.reviews[] | select(.author.login | endswith("[bot]")) | {author: .author.login, body: .body}'`

This returns the review body for every bot reviewer. For CodeRabbit in particular, look for sections titled "🟠 Major comments", "🟡 Minor comments", and "⚠️ Outside diff range comments" — these contain findings that were NOT posted as inline comments.

Parse each finding from the review body the same way as inline comments: extract the file path, line range, severity, title, description, and suggested fix. Treat each as an independent finding to evaluate.

If the review body output is very long (the shell may truncate it), save it to a file:

```bash
gh pr view --json reviews --jq '.reviews[] | select(.author.login | endswith("[bot]")) | .body' > /tmp/pr_review_body.txt
```

Then read `/tmp/pr_review_body.txt` to process all findings.

If no bot review bodies are found (output empty), continue to Step 3.
